<?php
    require('./funcionesBD.php');
    
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/styles.css">
    <title>Índice</title>
</head>
<body>
    <header>Gestion de base de datos</header>
    <div id="indice">
        <form action="./redireccion.php"  method ="post" enctype="multipart/form-data">  
            <?php
                if(compruebaBase()!='peliculas'){
            ?>
                    <input type="submit" value="Crear Base de datos" name="crear">
            <?php
                }
            ?>
            <input type="submit" value="Leer Tabla" name="leer">
            <input type="submit" value="Editar Tabla" name="editar">
        </form>
    </div>


    <footer>
        <a href="./verPag.php?pagina=index.php">Ver Código</a>
        <a href="./verPag.php?pagina=redireccion.php">Ver Código Redireccion.sql</a>
    </footer>
</body>
</html>