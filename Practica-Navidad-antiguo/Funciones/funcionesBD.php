...
function nuevoProducto($descripcion,$precio,$stock){
    try{
        $con = new PDO('mysql:host='.HOST.';dbname='.BBDD,USER,PASS);
        $sql='insert into productos values(:id,:nombre,:precio,:stock);';
        $preparada = $con->prepare($sql);
        $array = array(":id"=>null,":nombre"=>$descripcion,":precio"=>$precio,":stock"=>$stock);
        $preparada->execute($array);
...
function modProducto($id,$descripcion,$precio){
    try{
        ...
        $preparada = $con->prepare($sql);
        $array = array(":id"=>$id,":descripcion"=>$descripcion,":precio"=>$precio,":stock"=>$stock);
        $preparada->execute($array);
...
function modVenta($id,$usuario,$fecha,$cod_producto,$cantidad,$precio){
    try{
        ...
        $preparada = $con->prepare($sql);
        $array = array(":id"=>$id,":usuario"=>$usuario,":fecha"=>$fecha,":cod_producto"=>$cod_producto,":cantidad"=>$cantidad,":precio"=>$precio);
        $preparada->execute($array);
