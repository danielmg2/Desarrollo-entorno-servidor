...
function leeProductos(){
    try{
        ...
        $preparada->bindColumn(6,$img);



        // if(($_SESSION==null)||($_SESSION['perfil']=='usuario')){
            ?>
                                            ...
                                            <div class="btn-group">
                                                <form action="./Funciones/redireccion.php"  method ="post" enctype="multipart/form-data">
                                                        ...
                                                        if($_SESSION!=null){
                                                            ...
                                                            <label for="cantidad">Cantidad:</label>
                                                                    <input type="number" name="cantidad" id="cantidad" value="1" min="1" max="<?php echo $stock ?>">  
                                                                    <input type="submit" value="comprar" name="iSesion">
                                                                    <input type="hidden" name="clave" value="<?php echo $id ?>">
                                                                    <input type="hidden" name="precio" value="<?php echo $precio ?>">
                                                            <?php
                                                        }
...
function nuevoProducto($nombre,$descripcion,$precio,$stock,$img){
    try{
        $con = new PDO('mysql:host='.HOST.';dbname='.BBDD,USER,PASS);
        $sql='insert into productos values(:id,:nombre,:descripcion,:precio,:stock,:img);';
        $preparada = $con->prepare($sql);
        $array = array(":id"=>null,":nombre"=>$nombre,":descripcion"=>$descripcion,":precio"=>$precio,":stock"=>$stock,":img"=>$img);
        $preparada->execute($array);
...
function modProducto($id,$nombre,$descripcion,$precio,$stock,$img){
    try{
        ...
        $sql = 'update productos set nombre = :nombre, descripcion = :descripcion, precio = :precio, stock= :stock, imagen= :img  where id= :id';
        $preparada = $con->prepare($sql);
        $array = array(":id"=>$id,":nombre"=>$nombre,":descripcion"=>$descripcion,":precio"=>$precio,":stock"=>$stock,":img"=>$img);
        $preparada->execute($array);
